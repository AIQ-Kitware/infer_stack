...
def validate_resolved(resolved: dict[str, Any]) -> dict[str, Any]:
    ...
    if backend == "kubeai":
        ...
        if reverse_proxy.get("enabled"):
            errors.append("backend=kubeai does not render the Compose reverse proxy frontend")
    ...
    if reverse_proxy.get("enabled"):
        ...
        if target in {"open_webui", "open-webui"} and not open_webui.get("enabled"):
            errors.append("reverse_proxy target=open_webui but Open WebUI is disabled")
        if target == "litellm" and not litellm.get("enabled"):
            errors.append("reverse_proxy target=litellm but LiteLLM is disabled")
        ...
        if config_path:
            ...
            for field in ("certificate", "certificate_key"):
                ...
                if not value:
                    warnings.append(f"reverse_proxy SSL is enabled but ssl.{field} is empty")
    ...
    if backend == "compose" and open_webui.get("enabled") and open_webui.get("publish_port", True):
    ...
    if backend == "compose" and reverse_proxy.get("enabled"):
        if reverse_proxy.get("publish_http", True):
            port = int(reverse_proxy.get("http_port") or resolved.get("ports", {}).get("reverse_proxy_http", 80))
            if port in used_ports:
                errors.append(f"duplicate host port assignment: {port}")
            used_ports.add(port)
        if reverse_proxy.get("publish_https", True):
            port = int(reverse_proxy.get("https_port") or resolved.get("ports", {}).get("reverse_proxy_https", 443))
            if port in used_ports:
                errors.append(f"duplicate host port assignment: {port}")
            used_ports.add(port)
