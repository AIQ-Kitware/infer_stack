...
def _compose_gpus(value: object, indent: int = 4) -> str:
    ...
    if isinstance(value, (str, int, float)):
        scalar = (
            _compose_quote(value)
            if isinstance(value, str)
            else json.dumps(value)
        )
        return f'{pad}gpus: {scalar}'
    dumped = yaml.safe_dump(
        {'gpus': value}, default_flow_style=False, sort_keys=False
    ).rstrip('\n')
    return '\n'.join(pad + line for line in dumped.splitlines())


def render_compose_artifacts(
    lock_data: dict, *, assume_yes: bool = True
) -> None:
    ...
    if (frontends.get('open_webui') or {}).get('enabled'):
        env_values.update(
            {
                'OPENWEBUI_POSTGRES_DB': existing.get(
                    'OPENWEBUI_POSTGRES_DB', 'openwebui'
                ),
                'OPENWEBUI_POSTGRES_USER': existing.get(
                    'OPENWEBUI_POSTGRES_USER', 'openwebui'
                ),
                'OPENWEBUI_POSTGRES_PASSWORD': ensure_secret(
                    existing, 'OPENWEBUI_POSTGRES_PASSWORD'
                ),
                'WEBUI_SECRET_KEY': ensure_secret(existing, 'WEBUI_SECRET_KEY'),
            }
        )
        ldap_defaults = (
            (frontends.get('open_webui') or {}).get('ldap') or {}
        ).get('env_defaults') or {}
    ...
    if (gateways.get('litellm') or {}).get('enabled'):
        env_values.update(
            {
                'LITELLM_POSTGRES_DB': existing.get(
                    'LITELLM_POSTGRES_DB', 'litellm'
                ),
                'LITELLM_POSTGRES_USER': existing.get(
                    'LITELLM_POSTGRES_USER', 'litellm'
                ),
                'LITELLM_POSTGRES_PASSWORD': ensure_secret(
                    existing, 'LITELLM_POSTGRES_PASSWORD'
                ),
                'LITELLM_MASTER_KEY': ensure_secret(
                    existing, 'LITELLM_MASTER_KEY', prefix='sk-'
                ),
            }
        )
    ...
    if (providers.get('vllm') or {}).get('enabled'):
        env_values.update(
            {
                'VLLM_BACKEND_API_KEY': ensure_secret(
                    existing, 'VLLM_BACKEND_API_KEY'
                ),
    ...
    # LiteLLM port
    if (gateways.get('litellm') or {}).get('enabled'):
        litellm_port = ports.get('litellm') or DEFAULT_PORTS.get(
            'litellm', 14042
        )
        env_values['INFER_STACK_LITELLM_PORT'] = existing.get(
            'INFER_STACK_LITELLM_PORT', str(litellm_port)
        )

    # Open WebUI port
    if (frontends.get('open_webui') or {}).get('enabled'):
        open_webui_port = ports.get('open_webui') or DEFAULT_PORTS.get(
            'open_webui', 13000
        )
        env_values['INFER_STACK_OPEN_WEBUI_PORT'] = existing.get(
            'INFER_STACK_OPEN_WEBUI_PORT', str(open_webui_port)
        )
    ...
    if reverse_proxy.get('enabled'):
        http_port = (
            ...
            or DEFAULT_PORTS.get('reverse_proxy_http', 80)
        )
        https_port = (
            reverse_proxy.get('https_port')
            or ports.get('reverse_proxy_https')
            or DEFAULT_PORTS.get('reverse_proxy_https', 443)
        )
        env_values['INFER_STACK_REVERSE_PROXY_HTTP_PORT'] = existing.get(
            'INFER_STACK_REVERSE_PROXY_HTTP_PORT', str(http_port)
        )
        env_values['INFER_STACK_REVERSE_PROXY_HTTPS_PORT'] = existing.get(
            'INFER_STACK_REVERSE_PROXY_HTTPS_PORT', str(https_port)
        )
    ...
    if (providers.get('ollama') or {}).get('enabled'):
        ...
        ollama_port = (
            (providers.get('ollama') or {}).get('host_port')
            or ports.get('ollama')
            or DEFAULT_PORTS.get('ollama', 11434)
        )
        env_values['INFER_STACK_OLLAMA_PORT'] = existing.get(
            'INFER_STACK_OLLAMA_PORT', str(ollama_port)
        )
    ...
    reverse_proxy = (deployment.get('frontends') or {}).get(
        'reverse_proxy'
    ) or {}
    if reverse_proxy.get('enabled'):
    ...
    compose = (
        env.from_string(_template('docker-compose.yml.j2')).render(**ctx) + '\n'
    )
    ...
    if (gateways.get('litellm') or {}).get('enabled'):
        litellm_cfg = (
            env.from_string(_template('litellm_config.yaml.j2')).render(**ctx)
            + '\n'
        )
    ...
    if reverse_proxy.get('enabled') and not reverse_proxy.get('config_path'):
        ...
        planned[Path(reverse_proxy['nginx_config_path'])] = (
            env.from_string(_template('nginx.conf.j2')).render(**nginx_ctx)
            + '\n'
        )

    if not confirm_writes(
        planned, assume_yes=assume_yes, title='Pending compose render'
    ):
