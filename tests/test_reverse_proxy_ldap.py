...
def test_openwebui_tls_ldap_profile_renders_ldap_and_reverse_proxy(
    tmp_path: Path,
) -> None:
    dep = resolve(
        _cfg(tmp_path, 'openwebui-tls-ldap'),
    ...
    assert (
        services['open-webui']['environment']['LDAP_SERVER_HOST']
        == '${LDAP_HOST}'
    )
...
def test_reverse_proxy_can_use_manual_nginx_config_path(tmp_path: Path) -> None:
    ...
    (tmp_path / 'manual-nginx.conf').write_text(
        'server { listen 80; }\n', encoding='utf-8'
    )
    ...
    assert (
        f'{tmp_path / "manual-nginx.conf"}:/etc/nginx/conf.d/default.conf:ro'
        in volumes
    )
...
def _ollama_openwebui_profile(
    reverse_proxy: dict | None = None, open_webui: dict | None = None
) -> dict:
    frontends: dict = {
        'open_webui': {
            'enabled': True,
...
def test_http_only_reverse_proxy_has_no_tls_block(tmp_path: Path) -> None:
    ...
    assert compose_doc['services']['reverse-proxy']['ports'] == [
        '${INFER_STACK_REVERSE_PROXY_HTTP_PORT:-80}:80'
    ]
    nginx_text = (tmp_path / 'state' / 'runtime' / 'nginx.conf').read_text()
...
def test_force_https_redirect_present_with_tls(tmp_path: Path) -> None:
    rp = {
        ...
        'ssl': {
            ...
            'certificate_key': 'key.key',
        },
    }
    cfg = _cfg_with_profile(
        tmp_path, 'tls', _ollama_openwebui_profile(reverse_proxy=rp)
    )
...
def test_ldap_disabled_omits_ldap_env(tmp_path: Path) -> None:
    ...
    assert not any(
        key.startswith('LDAP_') or key == 'ENABLE_LDAP' for key in env
    )
...
def test_generic_service_escape_hatches_render(tmp_path: Path) -> None:
    ...
    cfg = _cfg_with_profile(
        tmp_path, 'hatches', _ollama_openwebui_profile(open_webui=open_webui)
    )
    compose_doc = _render(cfg, tmp_path)
...
def test_extra_config_injected_into_nginx(tmp_path: Path) -> None:
    ...
    cfg = _cfg_with_profile(
        tmp_path, 'extra-config', _ollama_openwebui_profile(reverse_proxy=rp)
    )
    _render(cfg, tmp_path)
    nginx_text = (tmp_path / 'state' / 'runtime' / 'nginx.conf').read_text()
...
def test_no_watchtower_service_is_rendered(tmp_path: Path) -> None:
    ...
    assert not any(
        (svc or {}).get('image', '').startswith('nickfedor/watchtower')
...
def test_validator_warns_on_missing_certs(tmp_path: Path) -> None:
    rp = {
        ...
        'ssl': {
            'enabled': True,
            'certificate': '',
            'certificate_key': 'missing.key',
        },
    }
    cfg = _cfg_with_profile(
        tmp_path, 'missing-certs', _ollama_openwebui_profile(reverse_proxy=rp)
    )
    dep = resolve(cfg, inventory=simulate_inventory('1x24'))
    report = validate_resolved(dep)
...
def test_validator_errors_on_publish_https_without_ssl_in_plan(
    tmp_path: Path,
) -> None:
...
def test_validator_warns_on_missing_config_path(tmp_path: Path) -> None:
    ...
    cfg = _cfg_with_profile(
        tmp_path, 'missing-config', _ollama_openwebui_profile(reverse_proxy=rp)
    )
    dep = resolve(cfg, inventory=simulate_inventory('1x24'))
    report = validate_resolved(dep)
    assert report['ok'] is True
    assert any(
        'config_path' in w and 'not found' in w for w in report['warnings']
    )
