...
def test_setup_compose_then_render_without_manual_file_edits(
    tmp_path: Path,
) -> None:
    run_cli(
        tmp_path,
...
def test_setup_kubeai_then_render_without_manual_file_edits(
    tmp_path: Path,
) -> None:
    run_cli(
        tmp_path,
...
def test_setup_kubeai_with_resource_profiles_file_syncs_canonical_values(
    tmp_path: Path,
) -> None:
    ...
    values_doc = yaml.safe_load(
        (tmp_path / 'kubeai-values.local.yaml').read_text()
    )
    assert 'gpu-single-default' in values_doc['resourceProfiles']
    assert (
        values_doc['resourceProfiles']['gpu-single-default']['extraField']
        == 'keep-me'
    )


def test_render_overrides_backend_and_profile_without_persisting_config(
    tmp_path: Path,
) -> None:
    run_cli(
        tmp_path,
    ...
    namespace_doc = yaml.safe_load(
        (tmp_path / 'generated' / 'kubeai' / 'namespace.yaml').read_text()
    )
...
def test_kubeai_sync_resource_profiles_then_validate_without_config_duplication(
    tmp_path: Path,
) -> None:
    source = tmp_path / 'values-kubeai-local-gpu.yaml'
    write_kubeai_values_file(source)
    ...
    run_cli(
        tmp_path, 'kubeai-sync-resource-profiles', '--from-file', str(source)
    )
    run_cli(tmp_path, 'validate', '--simulate-hardware', '1x96')


def test_kubeai_config_fallback_still_works_before_and_after_render_without_sync(
    tmp_path: Path,
) -> None:
    run_cli(
        tmp_path,
    ...
    cfg = yaml.safe_load((tmp_path / 'config.yaml').read_text())
    cfg['resource_profiles']['gpu-single-default']['node_selector'] = {
        'example.com/profile-source': 'config-after-render'
    }
    (tmp_path / 'config.yaml').write_text(
        yaml.safe_dump(cfg, sort_keys=False), encoding='utf-8'
    )
    run_cli(tmp_path, 'validate', '--simulate-hardware', '1x96')
    run_cli(tmp_path, 'render', '--yes', '--simulate-hardware', '1x96')
    rendered_values = yaml.safe_load(
        (tmp_path / 'generated' / 'kubeai' / 'kubeai-values.yaml').read_text()
    )
    assert (
        rendered_values['resourceProfiles']['gpu-single-default'][
            'nodeSelector'
        ]['example.com/profile-source']
        == 'config-after-render'
    )


def test_kubeai_synced_source_is_preferred_over_config_and_preserves_unknown_fields(
    tmp_path: Path,
) -> None:
    source = tmp_path / 'values-kubeai-local-gpu.yaml'
    write_kubeai_values_file(source)
    ...
    cfg = yaml.safe_load((tmp_path / 'config.yaml').read_text())
    cfg['resource_profiles']['gpu-single-default']['node_selector'] = {
        'example.com/profile-source': 'config'
    }
    (tmp_path / 'config.yaml').write_text(
        yaml.safe_dump(cfg, sort_keys=False), encoding='utf-8'
    )
    ...
    assert (
        generated_values['resourceProfiles']['gpu-single-default'][
            'nodeSelector'
        ]['nvidia.com/gpu.product']
        == 'NVIDIA_TEST_GPU'
    )
    assert (
        generated_values['resourceProfiles']['gpu-single-default']['extraField']
        == 'keep-me'
    )
    assert 'example.com/profile-source' not in generated_values[
        'resourceProfiles'
    ]['gpu-single-default'].get('nodeSelector', {})


def test_kubeai_local_values_change_marks_render_stale(
    ...
    values_doc['resourceProfiles']['gpu-single-default']['nodeSelector'][
        'example.com/profile-source'
    ] = 'updated-local'
    (tmp_path / 'kubeai-values.local.yaml').write_text(
        yaml.safe_dump(values_doc, sort_keys=False), encoding='utf-8'
    )
    assert cli_mod.render_is_stale() is True


def test_kubeai_deploy_rerenders_from_updated_local_values(
    tmp_path: Path, monkeypatch
) -> None:
    source = tmp_path / 'values-kubeai-local-gpu.yaml'
    write_kubeai_values_file(source)
    ...
    monkeypatch.setattr(
        cli_mod.commands_runtime,
        'deploy_rendered_artifacts',
        fake_deploy_rendered_artifacts,
    )
...
def test_openwebui_state_paths_are_not_deleted_or_reinitialized_on_switch(
    ...
    for sentinel in (
        sentinel_open_webui,
        sentinel_postgres_owui,
        sentinel_postgres_litellm,
    ):
...
def test_kubeai_status_namespace_error_is_actionable(
    tmp_path: Path, monkeypatch
) -> None:
    run_cli(
        tmp_path,
    ...
    monkeypatch.setattr(
        cli_mod.commands_runtime, 'kubeai_print_status', fake_status
    )
    try:
...
def test_smoke_test_uses_chat_endpoint_for_chat_profiles(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    def fake_get(url, **kwargs):
        return _FakeResponse(
            {'data': [{'id': 'qwen2-5-7b-instruct-turbo-default'}]}
        )
    ...
    cli_mod.SmokeTestCLI.main(
        argv=False,
        **_smoke_test_kwargs(model='qwen2-5-7b-instruct-turbo-default'),
    )
...
def test_smoke_test_uses_completions_endpoint_for_completions_profiles(
    ...
    monkeypatch.setattr(cli_mod.requests, 'post', fake_post)
    cli_mod.SmokeTestCLI.main(
        argv=False, **_smoke_test_kwargs(model='eleutherai/pythia-1b-v0')
    )
...
def test_smoke_test_protocol_override_forces_completions(
    ...
    cli_mod.SmokeTestCLI.main(
        argv=False,
        **_smoke_test_kwargs(
            model='qwen2-5-7b-instruct-turbo-default', protocol='completions'
        ),
    )
    assert posted['url'].endswith('/completions')
    assert 'prompt' in posted['json']
